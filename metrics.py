...
def compare_mpsnr(x_true, x_pred, data_range):
    ...
    total_psnr = [compare_psnr(x_true[:, :, k], x_pred[:, :, k], data_range=data_range) for k in range(x_true.shape[2])]

    return np.mean(total_psnr)

def compare_mpsnr_test(x_true, x_pred, data_range):
    """
    :param x_true: Input image must have three dimension (H, W, C)
    :param x_pred:
    :return:
    """
    x_true, x_pred = x_true.astype(np.float32), x_pred.astype(np.float32)
    print(np.argwhere(np.isnan(x_true)))
    print(np.argwhere(np.isnan(x_pred)))
    channels = x_true.shape[2]
    total_psnr = [compare_psnr(im_true=x_true[:, :, k], im_test=x_pred[:, :, k], data_range=data_range)
                  for k in range(channels)]
...
def compare_mssim(x_true, x_pred, data_range, multidimension):
    ...
    :param multidimension: 是否是多通道图像
    :return: 返回图像的SSIM平均值
    """
    mssim = [compare_ssim(im1=x_true[:, :, i], im2=x_pred[:, :, i], data_range=data_range, multichannel=multidimension)
             for i in range(x_true.shape[2])]  # 循环遍历每个通道

    return np.mean(mssim)  # 返回平均的SSIM值
